...
class Ksiazka{	//definicja klasy
	private: //składowe są prywatne - dostępne tylko z operacji klasy
		int id;
		int rok_wydania;
		int liczba_stron;
		int stan;
		string tytul;	//zmienna składowa klasy (dana); string - łańcuch znaków
		string autor;
		float cena;
	public:	 //składowe publiczne - można ich używać spoza klasy(np funkcji main)
		void wpisz(){	//funkcja składowa klasy (operacja)
			cout<<"\nPodaj informacje o ksiazce: \n";
			cout<<"Id: ";
			
			//cin.sync();
			cin>>id;	//ID powinno być nadawane automatycznie
			cout<<"Tytul: ";
			cin>>ws;
			getline(cin, tytul);
			cout<<"Autor: ";
			...
			do{
				...
				cin.sync(); // opróżnia bufor strumienia
				cout<<"Rok wydania: ";
				cin>>rok_wydania;	
			}while(cin.fail());
			do{
			...
			do{
				...
				cin>>stan;
				if(stan<1||stan>5){
					cout<<"Wprowadzono nieprawidłowy stan ksiazki. Sprobuj jeszcze raz.\n";
					p=1;
				}
			}while(cin.fail()||p);
		}
...
int dodaj_ksiazki(int n, Ksiazka ksiazki[]){
	...
	char decyzja = 't';
	do{
		ksiazki[n].wpisz(); //wywołanie metody wpisz dla obiektu n-tego w tablicy
		n++;
...
void usuwanie_informacji(int n, Ksiazka ksiazki[]){
	...
	//mozna dodać jeszcze automatyczne numerowanie ID
...
void sortowanie(int n, Ksiazka ksiazki[]){
	//sortowanie nie jest "odporne" na tytuły i autorów wprowadzonych raz z małych raz z wielkich liter (kod ASCII)
	char d;
	int pmin, pmax, i, p;
	bool j;
...
int main(){
	...
	Ksiazka ksiazki[100]; //utworzenie tablicy, której elementy są obiektami klasy ksiażka;
	int lp_ksiazek=0;
	int w;
	bool k=true;
